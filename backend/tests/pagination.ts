...
async function main() {
  ...
  const register = async (label: string) => { const password = `Qa-${randomBytes(12).toString('base64url')}9!`; const result = await request('POST', '/auth/register', { firstName: 'QA', lastName: label, email: `pagination-${label}-${suffix}@example.com`, password, businessName: `Pagination ${label}` }); assert.equal(result.status, 201); businesses.push(result.body.user.business.id); return result.body.token as string }
  try {
