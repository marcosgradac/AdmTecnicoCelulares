...
import type { BusinessSettings, SettingsData } from './settings.types'
export const getSettings = async () => (await api.get<SettingsData>('/settings')).data
export const updateBusiness = async (input: BusinessSettings) => (await api.patch<BusinessSettings>('/settings/business', input)).data
export const requestPasswordCode = async () => (await api.post<{ success: true; maskedEmail: string; retryAfter: number }>('/auth/password-change/request')).data
