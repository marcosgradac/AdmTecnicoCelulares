...
export function EquipmentSalesPage() {
  ...
  return <Box>
    ...
    <Card><CardContent>
      ...
      <Stack direction="row" alignItems="center" flexWrap="wrap" gap={1} mb={2} aria-label="Flujo del equipo">{(Object.keys(statusLabels) as EquipmentStatus[]).map((status, index) => <Stack key={status} direction="row" alignItems="center" spacing={1}><Chip size="small" variant="outlined" color={statusColors[status]} label={statusLabels[status]} />{index < 3 && <ArrowForwardRounded sx={{ fontSize: 15, color: 'text.disabled' }} />}</Stack>)}</Stack>
