...
import { ContentCopyRounded, EditRounded, PaymentsRounded, WhatsApp } from '@mui/icons-material'
...
export function RepairDetailPage() {
  ...
  const [form, setForm] = useState<UpdateRepairInput | null>(null)
  const [paymentOpen, setPaymentOpen] = useState(false)
  const [payment, setPayment] = useState({ amount: 0, method: 'TRANSFER' as 'CASH' | 'TRANSFER' | 'CARD' | 'OTHER' })
  ...
  const saveChanges = async () => {
    ...
    try {
      const updated = await updateRepair(repair.id, form)
  ...
  const savePayment = async () => {
    ...
    try { await registerPayment(repair.id, payment); await load(); setPaymentOpen(false); setPayment({ amount: 0, method: 'TRANSFER' }); setSuccess('Pago registrado correctamente.') }
    catch (paymentError) { setError(messageFrom(paymentError, 'No pudimos registrar el pago.')) }
    finally { setSaving(false) }
  }
  const setField = <K extends keyof UpdateRepairInput>(key: K, value: UpdateRepairInput[K]) => setForm(current => current ? { ...current, [key]: value } : current)
  ...
  return <Box>
    ...
    {editing && form ? <Card sx={{ mb: 2.2 }}><CardContent>
      ...
      <Grid container spacing={2}>
        ...
        <Grid size={{ xs: 12, md: 4 }}><TextField required fullWidth type="number" label="Total del presupuesto" value={form.total} onChange={event => setField('total', Number(event.target.value))} helperText={`Pagado: ${formatMoney(repair.paid)}`}/></Grid>
      ...
      <Stack direction="row" justifyContent="flex-end" gap={1} mt={2}><Button onClick={cancelEditing} disabled={saving}>Cancelar</Button><Button variant="contained" disabled={saving || !form.deviceBrand.trim() || !form.deviceModel.trim() || form.issue.trim().length < 2} onClick={() => void saveChanges()}>{saving ? 'Guardando…' : 'Guardar cambios'}</Button></Stack>
    </CardContent></Card> : null}
    <Grid container spacing={2.2}>
      <Grid size={{ xs: 12, lg: 8 }}><Stack spacing={2.2}>
        <RepairPartsSection repairId={repair.id} repairTotal={repair.total}/>
      ...
      <Grid size={{ xs: 12, lg: 4 }}><Stack spacing={2.2}><Card><CardContent><Typography variant="h2">Equipo y cliente</Typography><Stack spacing={1.7} mt={2}><Info label="Equipo" value={repair.device}/><Info label="Color" value={repair.color || 'No informado'}/><Info label="IMEI / serie" value={repair.imei || 'No informado'}/><Info label="Cliente" value={repair.clientName}/><Info label="WhatsApp" value={repair.phone || 'No informado'}/></Stack></CardContent></Card><Card><CardContent><Stack direction="row" gap={1}><PaymentsRounded color="primary"/><Typography variant="h2">Resumen de pago</Typography></Stack><Stack spacing={1.6} mt={2}><Info label="Total" value={formatMoney(repair.total)}/><Info label="Pagado" value={formatMoney(repair.paid)} color="success.main"/><Divider/><Info label="Saldo" value={formatMoney(Math.max(0, repair.total - repair.paid))} color="error.main"/></Stack><Button fullWidth variant="outlined" sx={{ mt: 2 }} disabled={repair.paid >= repair.total} onClick={() => setPaymentOpen(true)}>Registrar pago</Button></CardContent></Card></Stack></Grid>
    </Grid>
    <Dialog open={paymentOpen} onClose={() => setPaymentOpen(false)} fullWidth maxWidth="xs"><DialogTitle>Registrar pago</DialogTitle><DialogContent><Stack spacing={2} mt={1}><TextField type="number" label="Importe" value={payment.amount} onChange={event => setPayment(value => ({ ...value, amount: Number(event.target.value) }))}/><TextField select label="Método" value={payment.method} onChange={event => setPayment(value => ({ ...value, method: event.target.value as typeof value.method }))}><MenuItem value="CASH">Efectivo</MenuItem><MenuItem value="TRANSFER">Transferencia</MenuItem><MenuItem value="CARD">Tarjeta</MenuItem><MenuItem value="OTHER">Otro</MenuItem></TextField></Stack></DialogContent><DialogActions><Button onClick={() => setPaymentOpen(false)}>Cancelar</Button><Button variant="contained" disabled={saving || payment.amount <= 0} onClick={() => void savePayment()}>Guardar pago</Button></DialogActions></Dialog>
  </Box>
}
