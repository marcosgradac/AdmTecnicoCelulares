...
export function ClientsPage() {
  ...
  return <Box><PageHeader eyebrow="RELACIONES" title="Clientes" description="Información de contacto e historial de cada cliente." action={<Button variant="contained" startIcon={<AddRounded />} onClick={() => setNewOpen(true)}>Nuevo cliente</Button>} /><Card><CardContent>
    ...
    {!loading && !items.length && <UiState title={search ? 'No encontramos resultados' : 'No hay clientes todavía'} description={search ? 'Probá cambiando la búsqueda.' : 'Agregá tu primer cliente para comenzar a trabajar con TecnoDesk.'} action={search ? () => setQuery('') : () => setNewOpen(true)} actionLabel={search ? 'Limpiar filtros' : 'Nuevo cliente'} />}
