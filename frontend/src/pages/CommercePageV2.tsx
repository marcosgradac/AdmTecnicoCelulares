...
import { Alert, Box, Button, Card, CardContent, Chip, Divider, Grid, IconButton, MenuItem, Stack, Table, TableBody, TableCell, TableHead, TablePagination, TableRow, TextField, Typography } from '@mui/material'
...
export function CommercePageV2() {
  ...
  return <Box><PageHeader eyebrow="COMERCIO" title="Venta de accesorios y productos" description="Productos, stock y ventas de mostrador, separados de Reparaciones." action={<Stack direction="row" gap={1} flexWrap="wrap" justifyContent="flex-end"><Button variant="outlined" onClick={() => openCategory()} disabled={!canAccess(user, 'commerce.manage')}>Nueva categoría</Button><Button variant="outlined" startIcon={<Inventory2Rounded />} onClick={openNewProduct} disabled={!canAccess(user, 'commerce.manage')}>Nuevo producto</Button><Button variant="outlined" onClick={() => setExpenseOpen(true)} disabled={!canAccess(user, 'commerce.manage')}>Nuevo egreso</Button><Button variant="contained" startIcon={<PointOfSaleRounded />} onClick={() => setSaleOpen(true)} disabled={!canAccess(user, 'commerce.sell')}>Nueva venta</Button></Stack>} />
    ...
    <Card><CardContent><Stack direction="row" justifyContent="space-between" alignItems="center" mb={2}><Box><Typography variant="h2">Productos</Typography><Typography variant="body2" color="text.secondary">Buscá productos o usá Nueva venta para agregarlos al carrito.</Typography></Box><Chip label={`${totalProducts} productos`} icon={<Inventory2Rounded />} /></Stack><TextField label="Buscar productos" value={search} onChange={event => setSearch(event.target.value)} size="small" sx={{ mb: 2 }} />{loading ? <UiState loading /> : products.length ? <Table size="small"><TableHead><TableRow><TableCell>Producto</TableCell><TableCell>Categoría</TableCell><TableCell align="right">Precio</TableCell><TableCell align="right">Ganancia unidad</TableCell><TableCell align="right">Stock</TableCell><TableCell align="right">Acciones</TableCell></TableRow></TableHead><TableBody>{products.map(product => <TableRow key={product.id}><TableCell><Typography fontWeight={750}>{product.name}</Typography></TableCell><TableCell>{product.category}</TableCell><TableCell align="right">{formatMoney(product.salePrice)}</TableCell><TableCell align="right" color={product.unitProfit >= 0 ? 'success.main' : 'error.main'}>{formatMoney(product.unitProfit)}</TableCell><TableCell align="right"><Chip size="small" label={product.currentStock} /></TableCell><TableCell align="right"><RowActionsMenu label={`Acciones de ${product.name}`} actions={[{ label: 'Editar', icon: <EditRounded />, onClick: () => openEditProduct(product), disabled: !canAccess(user, 'commerce.manage') }, { label: 'Eliminar', icon: <DeleteOutlineRounded />, destructive: true, onClick: () => void removeProduct(product), disabled: !canAccess(user, 'commerce.manage') }]} /></TableCell></TableRow>)}</TableBody></Table> : <UiState title="Todavía no hay productos" description="Cargá tu primer accesorio para comenzar." action={canAccess(user, 'commerce.manage') ? openNewProduct : undefined} actionLabel="Nuevo producto"/>}{totalProducts > 20 && <TablePagination component="div" count={totalProducts} page={page} onPageChange={(_, next) => setPage(next)} rowsPerPage={20} rowsPerPageOptions={[20]} />}</CardContent></Card>
    <Card sx={{ mt: 2.2 }}><CardContent><Typography variant="h2" mb={2}>Ventas</Typography>{salesError && <Alert severity="error" sx={{ mb: 2 }} action={<Button color="inherit" disabled={salesLoading} onClick={() => setSalesRevision(value => value + 1)}>Reintentar ventas</Button>}>{salesError}</Alert>}{totalSales > 5 && <TablePagination component="div" count={totalSales} page={salesPage} rowsPerPage={5} rowsPerPageOptions={[5]} onPageChange={(_, next) => setSalesPage(next)} />}{salesLoading ? <UiState loading /> : sales.length ? <Stack divider={<Divider />} spacing={1}>{sales.map(sale => <Stack key={sale.id} direction="row" justifyContent="space-between" py={1}><Typography>{new Date(sale.createdAt).toLocaleString('es-AR')}</Typography><Typography fontWeight={750}>{formatMoney(sale.total)} · ganancia {formatMoney(sale.profit)}</Typography></Stack>)}</Stack> : !salesError && <Typography color="text.secondary">Todavía no hay ventas de mostrador.</Typography>}</CardContent></Card>
    ...
    <CategoryDrawer editing={!!editingCategory} error={categoryError} open={categoryOpen} name={categoryName} saving={saving} onName={setCategoryName} onClose={() => { if (!saving) setCategoryOpen(false) }} onSave={() => void saveCategory()} />
...
function SaleDrawer({ searchError, onRetry, open, error, search, onSearch, page, count, onPage, loading, products, selected, cartLines, total, paymentMethod, saving, onSelected, onAdd, onChange, onPayment, onClose, onSave }: { searchError: string; onRetry: () => void; open: boolean; error: string; search: string; onSearch: (value: string) => void; page: number; count: number; onPage: (page: number) => void; loading: boolean; products: CommerceProduct[]; selected: string; cartLines: Array<{ product: CommerceProduct; quantity: number }>; total: number; paymentMethod: CommercePaymentMethod; saving: boolean; onSelected: (value: string) => void; onAdd: () => void; onChange: (product: CommerceProduct, amount: number) => void; onPayment: (value: CommercePaymentMethod) => void; onClose: () => void; onSave: () => void }) { return <FormDrawer open={open} context="COMERCIO" title="Nueva venta" saving={saving} submitLabel="Confirmar venta" submitDisabled={!cartLines.length} onClose={onClose} onSubmit={onSave}>{error && <Alert severity="error">{error}</Alert>}<TextField label="Buscar productos" value={search} onChange={event => onSearch(event.target.value)} />{searchError && <Alert severity="error" action={<Button onClick={onRetry}>Reintentar</Button>}>{searchError}</Alert>}{!loading && !searchError && !products.length && <Alert severity="info">{search.trim() ? 'No hay productos con stock que coincidan con tu búsqueda.' : 'No hay productos con stock disponibles.'}</Alert>}<TablePagination component="div" count={count} page={page} rowsPerPage={20} rowsPerPageOptions={[20]} onPageChange={(_, next) => onPage(next)} /><Stack direction="row" spacing={1} alignItems="flex-start"><TextField fullWidth select disabled={loading || saving} label="Producto" value={selected} onChange={event => onSelected(event.target.value)}><MenuItem value="">Seleccionar producto</MenuItem>{products.map(product => <MenuItem key={product.id} value={product.id}>{product.name} · {formatMoney(product.salePrice)} · stock {product.currentStock}</MenuItem>)}</TextField><Button variant="outlined" startIcon={<AddRounded />} onClick={onAdd} disabled={!selected || loading || saving}>Agregar</Button></Stack>{cartLines.length ? <Stack spacing={1}>{cartLines.map(line => <Stack key={line.product.id} direction="row" alignItems="center" justifyContent="space-between"><Box><Typography fontWeight={700}>{line.product.name}</Typography><Typography variant="caption">{formatMoney(line.product.salePrice)} por unidad</Typography></Box><Stack direction="row" alignItems="center"><IconButton size="small" onClick={() => onChange(line.product, -1)}><RemoveRounded /></IconButton><Typography width={24} textAlign="center">{line.quantity}</Typography><IconButton size="small" onClick={() => onChange(line.product, 1)} disabled={line.quantity >= line.product.currentStock}><AddRounded /></IconButton></Stack></Stack>)}</Stack> : <Typography color="text.secondary">Agregá productos para preparar la venta.</Typography>}<Divider /><TextField select label="Medio de pago" value={paymentMethod} onChange={event => onPayment(event.target.value as CommercePaymentMethod)}>{paymentOptions.map(option => <MenuItem key={option[0]} value={option[0]}>{option[1]}</MenuItem>)}</TextField><Stack direction="row" justifyContent="space-between"><Typography variant="h2">Total</Typography><Typography variant="h2">{formatMoney(total)}</Typography></Stack></FormDrawer> }
...
function Metric({ label, value, tone }: { label: string; value: string; tone?: string }) { return <Grid size={{ xs: 6, lg: 3 }}><Card><CardContent><Typography variant="caption" color="text.secondary">{label}</Typography><Typography variant="h2" color={tone}>{value}</Typography></CardContent></Card></Grid> }
