...
export function AuthLayout({ title, description, children, variant = 'login' }: {
  ...
  const passwordFlow = variant === 'forgot-password' || variant === 'reset-password'

  return <Box className={`auth-page auth-page--${variant}`} minHeight="100vh" display="grid" sx={{
    gridTemplateColumns: { xs: '1fr', md: 'minmax(560px, 52%) minmax(520px, 48%)' },
    ...
    <Box className="auth-form-region" display="grid" px={{ xs: 3, sm: 4, md: 6 }} py={{ xs: 3, md: 5 }} sx={{ placeItems: 'center' }}>
      <Box className="auth-form-surface" sx={{ width: '100%', maxWidth: 460, borderRadius: 0, p: { xs: 2.5, sm: 4 }, bgcolor: 'transparent', border: 'none', boxShadow: 'none' }}>
        <BrandLogo className="auth-form-logo" />
        {variant !== 'login' && <Typography variant="h1" fontSize={{ xs: 27, sm: 32 }}>{title}</Typography>}
        <Typography color="text.secondary" mt={variant === 'register' || passwordFlow ? 0.75 : 1} mb={3}>{description}</Typography>
        <Stack className="auth-content-stack" spacing={variant === 'login' || passwordFlow ? 0 : 2}>{children}</Stack>
      </Box>
    </Box>
  </Box>
}
