...
export function AppShell() {
  ...
  const selected = (path: string) => path === '/inicio' ? location.pathname === path : location.pathname.startsWith(path)
  ...
  return <Box className={styles.shell}>
    ...
    <Menu anchorEl={accountAnchor} open={Boolean(accountAnchor)} onClose={closeAccount} slotProps={{ paper: { sx: { minWidth: 230, mt: 1 } } }}>
      ...
      <Divider />
      <MenuItem onClick={() => { closeAccount(); navigate('/perfil') }}><ListItemIcon><AccountCircleRounded fontSize="small" /></ListItemIcon>Mi perfil</MenuItem>
    ...
    {mobile && <Box component="nav" aria-label="Navegación principal" className={styles.mobileNav}>
      <button onClick={() => go('/inicio')} className={selected('/inicio') ? styles.active : ''}><DashboardRounded /><span>Inicio</span></button>
      <button onClick={() => go('/reparaciones')} className={selected('/reparaciones') ? styles.active : ''}><BuildRounded /><span>Reparaciones</span></button>
      <button onClick={() => go('/reparaciones/nueva')} className={styles.create}><AddRounded /><span>Nueva</span></button>
      <button onClick={() => go('/clientes')} className={selected('/clientes') ? styles.active : ''}><PeopleRounded /><span>Clientes</span></button>
